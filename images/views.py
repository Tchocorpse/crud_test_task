...
from images.serializers import ImageSerializer, CommentarySerializer, ImageSerializerWithId, CommentarySerializerWithId
...
class GetImages(generics.ListAPIView):
    ...
    serializer_class = ImageSerializerWithId
...
class GetCommentaries(generics.ListAPIView):
    ...
    serializer_class = CommentarySerializerWithId
...
class GetStat(APIView):
            ...
            status.HTTP_200_OK: openapi.Schema(
                ...
                properties={
                    ...
                    'unique_images_count': openapi.Schema(
                        type=openapi.TYPE_NUMBER, description="Total number of unique images"
                    ),
                    'comment_count': openapi.Schema(
                        type=openapi.TYPE_NUMBER, description="Total number of commentaries"
                    ),
                    'unique_comment_count': openapi.Schema(
                        type=openapi.TYPE_NUMBER, description="Total number of unique commentaries"
                    ),
                    'occupied_volume': openapi.Schema(
    ...
    def get(self, request):
        ...
        images_count, unique_images_count = self.counting_machine(images_qs, 'hash_value')

        comments_qs = Commentary.objects.all()
        comment_count, unique_comment_count = self.counting_machine(comments_qs, 'hash_value')

        occupied_volume = self.bytes_to_mb(self.measure_images_size(images_qs))
    ...
    def counting_machine(self, qs, unique_field):
        count = qs.count()
        unique_count = qs.distinct(unique_field).count()
        return count, unique_count

    def measure_images_size(self, qs):
